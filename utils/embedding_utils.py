...
class VideoEmbeddingGenerator:
    ...
    def generate_visual_embedding(self, frames):
        ...
        with torch.no_grad():
            # Explicitly passing pixel_values for compatibility with newer transformers versions
            image_embeddings = self.clip_model.get_image_features(pixel_values=frame_tensors)
            
            # Safety Check: Extract the actual tensor from the Box/Object if necessary
            if hasattr(image_embeddings, 'pooler_output'):
                image_embeddings = image_embeddings.pooler_output
            elif hasattr(image_embeddings, 'image_embeds'):
